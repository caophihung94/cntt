...
class MY_Model extends CI_Model {
    ...
    public function _get($param = NULL) {
        ...
        if (!empty($param['field_where_in']) && isset($param['param_where_in']) && is_array($param['param_where_in'])) {
            $this->db->where_in($param['field_where_in'], $param['param_where_in']);
        }
        if (isset($param['limit']) && (int) $param['limit'] > 0) {
            $this->db->limit((int) $param['limit'], (int) $param['start']);
    ...
    public function _save($param = NULL) {
        ...
        if ($flag == 0) {
            ...
            //update
            (isset($param['date']) && $param['date'] == TRUE) ? $data['updated'] = $time : '';
            $this->db->set($data);
    ...
    public function _del($param = NULL) {
        ...
        if (!empty($param['field_where_in']) && is_array($param['param_where_in'])) {
            $this->db->where_in($param['field_where_in'], $param['param_where_in']);
            $flag = $flag = + 1;
        }
        if ($flag > 0) {
